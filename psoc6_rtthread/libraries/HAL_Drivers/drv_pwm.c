...
{
    uint32_t channel = _CYHAL_TCPWM_CNT_NUMBER(htim->tcpwm.resource);
    uint32_t clock_hz = htim->tcpwm.clock_hz;
    ...
    period_counts = Cy_TCPWM_PWM_GetPeriod0(htim->tcpwm.base, channel) + 1U;
    pulse_counts = Cy_TCPWM_PWM_GetCompare0(htim->tcpwm.base, channel);

    configuration->period = (rt_uint32_t)((rt_uint64_t)period_counts * 1000000000ULL / clock_hz);
    configuration->pulse = (rt_uint32_t)((rt_uint64_t)pulse_counts * 1000000000ULL / clock_hz);
