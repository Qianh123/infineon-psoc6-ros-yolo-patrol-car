...
class VideoStreamHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        global CONTROL_MODE, EMERGENCY_STOP, NODE_REF

        path = urlparse(self.path).path

        if path == "/api/mode/manual":
            CONTROL_MODE = "manual"
            EMERGENCY_STOP = False
            if NODE_REF is not None:
                NODE_REF.send_cmd("car_stop", force=True)
                NODE_REF.state = "MANUAL"
                NODE_REF.close_count = 0
            return self._send_json({"ok": True, "mode": CONTROL_MODE})
        ...
        if path == "/api/manual/move":
            ...
            if speed not in [30, 50, 70, 90]:
                speed = 70

            if CONTROL_MODE != "manual":
                return self._send_json({"ok": False, "error": "not manual mode", "mode": CONTROL_MODE}, 409)

            cmd_map = {
                "forward": f"car_forward {speed}",
                "back": f"car_back {speed}",
                "left": f"car_left {speed}",
                "right": f"car_right {speed}",
                "stop": "car_stop",
            }

            cmd = cmd_map.get(direction, "car_stop")

            if NODE_REF is not None:
                NODE_REF.send_cmd(cmd, force=True)
        ...
        if path == "/api/stop":
            ...
            if NODE_REF is not None:
                NODE_REF.send_cmd("car_stop", force=True)
                NODE_REF.state = "STOPPED"
            return self._send_json({"ok": True, "action": "stop", "emergency_stop": True})
        ...
        if path == "/api/resume":
            ...
            if NODE_REF is not None:
                if CONTROL_MODE == "auto":
                    ...
                    NODE_REF.state = "MANUAL"
                    NODE_REF.send_cmd("car_stop", force=True)
            return self._send_json({"ok": True, "action": "resume", "mode": CONTROL_MODE})

        return self.do_GET()

    def do_GET(self):
        global LATEST_JPEG, LATEST_STATUS, CONTROL_MODE, EMERGENCY_STOP
        ...
        if path == "/api/status":
            ...
            data["emergency_stop"] = bool(EMERGENCY_STOP)

            if CONTROL_MODE == "manual":
                data["state"] = "KEYBOARD_CONTROL"
                data["reason"] = "manual_control"
        ...
        if path == "/" or path == "/index.html":
            body = b"""
            <html>
            <head><title>ROS2 YOLO Camera</title></head>
            <body style="background:#111;color:white;text-align:center;">
            <h2>ROS2 YOLO Running Camera</h2>
            <img src="/stream.mjpg" style="max-width:95%;border:2px solid #333;">
            </body>
            </html>
            """
            self.send_response(200)
...
def start_video_server():
    # 8090：视频流
    video_server = ThreadingHTTPServer(("0.0.0.0", 8090), VideoStreamHandler)
    video_thread = threading.Thread(target=video_server.serve_forever, daemon=True)
    video_thread.start()

    # 8091：保留备用 API
    api_server_8091 = ThreadingHTTPServer(("0.0.0.0", 8091), VideoStreamHandler)
    api_thread_8091 = threading.Thread(target=api_server_8091.serve_forever, daemon=True)
    api_thread_8091.start()

    # 8000：微信小程序专用 API，避开 8091 在开发者工具里的 timeout 问题
    api_server_8000 = ThreadingHTTPServer(("0.0.0.0", 8000), VideoStreamHandler)
    api_thread_8000 = threading.Thread(target=api_server_8000.serve_forever, daemon=True)
    api_thread_8000.start()

    print("[video] stream server started: http://0.0.0.0:8090")
    print("[api] backup api server started: http://0.0.0.0:8091")
    print("[api] miniprogram api server started: http://0.0.0.0:8000")
...
class PatrolNode(Node):
    ...
    def update_video_stream(self, frame, info=None):
        ...
        try:
            ...
            if info is not None:
                ...
                # 视频画面只保留一行小状态条，避免挡住画面
                if distance is not None:
                    dist_text = f"{distance:.1f}cm"
                else:
                    dist_text = "invalid"

                short_state = str(self.state)
                status_text = f"P:{person_count}  C:{cargo_count}  D:{dist_text}  {short_state}"

                # 小黑条，放在视频左上角，尽量小，不遮挡主体
                cv2.rectangle(show, (8, 8), (430, 38), (0, 0, 0), -1)
    ...
    def run(self):
        global NODE_REF, CONTROL_MODE, EMERGENCY_STOP
        NODE_REF = self
        last_time = time.time()
